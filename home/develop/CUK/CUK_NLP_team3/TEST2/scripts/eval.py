...
def eval_ppl(args):
    ...
    writer = SummaryWriter(log_dir=f"{args.ckpt}_eval_logs")
    ...
    with open(args.test_path, "r", encoding="utf-8") as f:
        for step, line in enumerate(f):
            ...
            samples_per_sec = (step + 1) / max(elapsed, 1e-6)
            writer.add_scalar("eval/loss", loss, step)
            writer.add_scalar("eval/samples_per_second", samples_per_sec, step)
...
def parse_args():
    ...
    p.add_argument("--mode", type=str, default="grammar",
                   choices=["grammar", "ppl"])
...
def key_from_meta(r):
    m = r.get("meta", {})
    s, o = m.get("s", {}), m.get("o", {})
    return (s.get("base"), o.get("base"), s.get("plural_type"),
            o.get("plural_type"), m.get("length"))
...
def eval_grammar(args):
    ...
    if not pairs:
        ...
        for r in rows:
            bucket[key_from_meta(r)]["ok" if r.get("label") == "ok"
                                     else "violation"].append(r)
    ...
    correct, gaps, confs = 0, [], []

    for step, (ok_ex, vi_ex) in enumerate(pairs):
        if args.use_instruction:
            ok_txt = build_prompt(ok_ex,
                                  condition=args.condition,
                                  for_eval=False,
                                  use_instruction=True)
            vi_txt = build_prompt(vi_ex,
                                  condition=args.condition,
                                  for_eval=False,
                                  use_instruction=True)
        else:
